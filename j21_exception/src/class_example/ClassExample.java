...
public class ClassExample {
	...
	public static void main(String[] args) {
		// class 의 설계 정보를 저장하는 객체 Class
		// Class 객체를 얻는 방법 3가지
		
		// 1. Object 의 정적 멤버인 class keyword 를 이용하여 객체 설계 정보 획득
		Class<Person> pClass1 = Person.class;
		// class 의 이름 정보 (package 를 포함하는 class 의 전체 이름)
		System.out.println(pClass1.getName());
		
		// 2. Object 의 인스턴스 멤버 메소드인 getClass()
		...
		// 3. class 의 이름 정보를 이용해서 설계 정보 획득
		// Class.forName("클래스 이름");
		
		try {
			Class<Person> pClass3 = (Class<Person>)Class.forName("class_example.Person");
			System.out.println(pClass3.getName()); 					// 패키지를 포함한 풀 네임
			System.out.println(pClass3.getSimpleName());			// class 이름
			System.out.println(pClass3.getPackage().getName()); 	// package 이름
			
			// class 에 정의된 Field(전역 변수) 정보
			...
			System.out.println("매개변수로 전달된 문자열 위치에 class 정보가 존재하지 않음.");
		} catch (InstantiationException e) {
			// 객체 설계 정보는 해당 위치에 존재하지만 객체 생성이 불가능할 때 발생하는 오류
			// 추상 클래스, interface 이러한 설계 정보를 가진 class 파일은
			// 자체 만으로 객체 생성이 불가하니 생성할 수 없다고 발생하는 예외
		} catch (IllegalAccessException e) {
			// class 정보도 존재하고 일반 class 이지만
			// 객체를 생성하기 위한 생성자를 호출할 수 없음.
			// 접근 제한자.
		} catch (IllegalArgumentException e) {
			// 선언된 생성자와 다른 타입의 인자값이 전달되었을 때 발생하는 예외
		} catch (InvocationTargetException e) {
			// newInstance 로 생성된 객체의 method 또는 생성자에서 예외가 발생되었을 때
			// 그 오류를 전달 - 생성자 내부에서 발생된 NullPointerException 등등
