	...
	public static void main(String[] args) {
		...
		while (true) {
			try {
				...
				System.out.println("입력하려는 정수를 한 번 더 입력해주세요.");
				String num = sc.next();
				
				int number = Integer.parseInt(num); // "삼"
				
				// 한 번 더 입력받아 일치하는 값일 때 배열에 값 추가
				if (score == number) {
					scores[index] = score;
				} else {
				...
				continue;
			} catch(InputMismatchException | NumberFormatException e) {	// java.util.InputMismatchException import
				System.out.println("정수가 입력되지 않음");
				sc.next(); // 정수가 아닌 사용되지 못한 입력값을 제거
				continue;
			} catch(Exception e) {
				// 발생된 예외에 대한 정보를 console 에 출력
