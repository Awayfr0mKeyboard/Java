...
public class StudentManagementIF {
		...
		while (isRun) {
			...
			if (selectNo == 1) {
				...
				for (int i = 0; i < students.length; i++) {
					...
					int num = i + 1;
					System.out.println(num + "번 째 학생 정보 입력");
					System.out.println("학생의 이름을 입력해 주세요.");
					// 공백을 포함한 한 줄의 문장을 입력 받음 'nextLine'
				...
				// 전체 합계(총점), 평균, 최고-최저 득점자 이름
				int total = 0;		// 총점
				double avg = 0;		// 평균
				// 최고-최저 점수를 가진 학생 정보를 저장할 변수
				Student stuMax, stuMin;
				// 비교를 위하여 배열 0번 째 인덱스의 학생 정보로 변수의 값 초기화
				...
				System.out.println("최고 득점자 : " + stuMax.name);
				System.out.println("최저 득점자 : " + stuMin.name);
