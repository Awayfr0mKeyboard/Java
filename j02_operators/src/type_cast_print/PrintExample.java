 ...
 * System.in : 실행된 운영체제의 console을 이용해서 값을 입력 받음
 * System.out : 실행된 운영체제의 console을 이용해서 값을 출력함
 * 
 * out.println(); ()안의 내용을 출력한 후 줄 바꿈
 * out.print(); ()안의 내용을 출력한 후 줄 바꿈하지 않음
...
public class PrintExample {
	...
	public static void main(String[] args) {
		...
		/*
		 	...
		 	printf - 줄 바꿈이 이루어지지 않음
		 	출력하려는 지지자(값)의 수 만큼 필요한 값을 알려줘야 함
		 	 ...
		 	 * %c - characted - 문자
		 	 * %s - string - 문자열
		 	 * %n - new line 줄 바꿈(line-feed, carriage return)
		...
		System.out.printf("%s님의 점수는 %d점 입니다. \n %n", name, score);
		
		/*
		 	Optional
		 	%[-][0][n][.m][,m]

		 	%[n] : 출력할 전체 자리 수를 n만큼 지정(오른쪽 정렬)
		 		%5d , 1000 == [] [1] [0] [0] [0]
		 	%[-] : 전체 자리 수가 지정된 경우 왼쪽으로 정렬하고 빈칸에 공백 출력
		 		%-5d , 1000 == [1] [0] [0] [0] []
		 	%[0] : 전체 자리 수가 지정되어 있을 때 빈 자리를 0으로 표현
		 		%05d , 1000 == [0] [1] [0] [0] [0]
		 	%[.m] : 소수점 아래 자리 수를 지정, 잘리는 소수점 자리는 반올림
		 		%.2f , 82.687 == 82.69
		 	%[,] : 정수의 자리수 지정
		...
		System.out.printf("%s교재의 가격은 %,020d입니다 %n", title, price);
