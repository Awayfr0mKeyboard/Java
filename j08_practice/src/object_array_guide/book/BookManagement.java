...
public class BookManagement {
	...
	public void  registerBook() {
		...
		for (int i = 0; i < books.length; i++) {
			if (books[i] == null) {
				...
				books[i].num = count;
				// 다음에 등록될 도서의 관리 번호 1 증가
				count++;
				*/
				break;		// 빈자리 찾아서 책이 등록되었다면 반복문 즉시 종료
			}
		}	// end for : books 배열 순회
	}	// end registBook
	...
	public void updateBook() {
		...
		int bookNum = getSelectNum("수정하실 책의 관리 번호를 입력하세요.");
		
		// 책 목록에서 관리 번호가 일치하는 책 정보를 반환
		...
		while (isUpdate) {
			...
			switch (no) {
				...
				case 3 :
					...
					// update while 문의 반복문 탈출 flag를 false로 변경하여 수정 반복문 탈출
					isUpdate = false;
				default :
					System.out.println("등록된 메뉴가 아닙니다.");
					return;
	...
	// 도서관리번호로 책 정보 찾기
	// 매개변수로 도서 관리 번호를 전달 받아
	// books 배열에 등록된 Book 객체 중에 일치하는 관리 번호를 가진 책 정보 반환
