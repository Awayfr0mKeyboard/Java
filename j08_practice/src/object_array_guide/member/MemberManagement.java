...
public class MemberManagement{
	...
	void join() {
		...
		// 입력한 정보가 사용 가능한 정보인지 확인
		// 사용 가능한 id == 중복 아이디 체크
		// 사용 가능한 비밀번호 pw와 rePw가 일치하는지 확인
		// if (memberIdCheck(id) == false)
		if (!memberIdCheck(id) || !pw.equals(rePw)) {
			System.err.println("이미 사용중인 아이디이거나 비밀번호가 일치하지 않습니다.");
	...
	void update() {
		...
		// 관리자 - 전체 회원 중에 회원 번호로 선택한 회원의 이름 정보 수정
