...
public class BankApplication {
	...
	void createAccount() {
		...
		String owner = sc.next();			// 계좌 소유주 이름을 문자열로 입력 받음
		
		System.out.print("계좌 번호 : ");
		String accountNumber = sc.next();	// 계좌 번호를 문자열로 입력 받음
		
		System.out.print("초기 입금액 : ");
		int balance = sc.nextInt();			// 초기 입금액을 정수 타입인 int 타입으로 입력받아 저장
		
		System.out.print("비밀번호 : ");
		String pass = sc.next();			// 비밀번호를 문자열로 입력받아 저장
		...
		// account[0] == null : 아직 계좌 정보가 등록되지 않은 항목 
	...
	void selectAccount() {
		...
		// 등록된 계좌 정보들 중에 계좌 번호와 비밀번호가 일치하는 Account 객체 조회
		// [newAccount] [null] [null] [null]
		for (int i = 0; i < account.length; i++) {
			// account[i] 항목에 할당된 Account 객체가 존재하는지 확인 (안하면 NullPointerException 발생)
			if (account[i] != null) {
				...
				if (account[i].ano.equals(accountNumber) && pass.equals(account[i].password)) {
					...
					// 계좌 번호와 비밀번호가 일치하는 계좌 정보를 콘솔에 출력
		...
		if (registAccount == null) {
			System.out.println("일치하는 계좌 정보가 존재하지 않습니다.");
		} else {
	...
	void withdraw() {
		...
		int money = sc.nextInt();
		// 계좌 잔고에서 money 만큼 차감한 금액을 result에 저장
	...
	Account findAccount(String ano, String password) {
		...
		for (int i = 0; i < account.length; i++) {
			if (account[i] != null && account[i].ano.equals(ano) && account[i].password.equals(password)) {
				// account[i] 번 째 항목에 계좌 정보가
				// 매개변수로 전달된 계좌 번호와 비밀번호가 일치하는 계좌 정보
				return account[i];
			}
		}	// end for
		return null;
