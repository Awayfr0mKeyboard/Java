...
public class CapitalQuiz {
	...
	private void input() {
		...
		a : while (true) {	// 사용자가 국가 이름에 q를 입력할 때까지 반복
			...
			System.out.printf("국가 입력 (현재 %d번 째 국가 등록 / q는 종료) >> ", (store.size()+1));
			...
			// contains 는 동등한 객체인지 비교를
			// Object equals method 로 비교
			...
			for (Nation n : store) {
				...
				// 사용자가 추가할려고 하는 국가 이름이 일치 - 중복 국가 등록
			...
			// for 문을 반복하는 동안 continue 를 만나지 않음
			Nation nation = new Nation(country, capitalcity);
			store.add(nation);
			System.out.println("국가 정보 추가 완료");
			 */
	...
	private void quiz() {
		...
		// 2번 메뉴 선택 시 list 에 저장된 국가 정보 중에
		// random 한 국가 정보 읽기
		// size() == 0
		//    0       1       2       3       4          7          
		...
		if (quiz.getCapital().equals(answer)) {
			...
			System.out.printf("아닙니다!!! %s의 수도는 %s입니다. %n", quiz.getCountry(), quiz.getCapital());
		}
