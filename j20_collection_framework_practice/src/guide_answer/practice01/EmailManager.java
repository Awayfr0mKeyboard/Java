            ...
            switch (choice) {
                case 1:
                    ...
                    if (isChecked) {
                    	...
                    	// 중복되는 이메일이 아닐 경우, set 에 사용자가 입력한 이메일 추가
                    ...
                    */
                    ...
                    /*
                    // emails set 저장된 이메일 정보 먼저 확인
                    for (String email : emails) {
                    	if (email.equals(emailToAdd)) {
                    		System.out.println("이메일 '" + emailToAdd + "'는 이미 존재합니다.");
                    		continue outter; // 이름이 outter 인 반복문으로 continue
                    	}
                    }
                    // 중복되는 이메일이 아닐 경우, set 에 사용자가 입력한 이메일 추가
                    emails.add(emailToAdd);
