...
public class TheaterReserveRowColumn {
	...
	public static void main(String[] args) {
		...
		*/
		...
		// 극장 예약 프로그램 종료 flag
		boolean isRun = true;
		// 예약 좌석 정보를 입력받기 위한 Scanner 선언 및 초기화
		...
		while (isRun) {
			...
			for (int i = 0; i < seats.length; i++) {
				...
				} // 내부 for end 한 행의 열 정보 출력
				System.out.println(" [" + (char)(i + 65) + "행]");
			} // 외부 for end 10개의 행 정보 출력, 상영관 그리기 완료, 좌석 표시 끝
			...
			// 좌석 열 번호 입력받기 1 ~ 10
			System.out.println("예약하실 좌석의 열 번호를 입력해 주세요. (1 ~ 10)");
			...
			// 정상적인 행과 열 정보 입력 시 출력
			System.out.printf("선택하신 좌석은 %s행 %d열입니다. %n", inputRow, inputColumn);
			
			// 중복 예약인지 확인
			// inputRow == A == 65 == 0
			// inputRow == J == 74 == 9
			int row = inputRow - 65;	// char 알파벳에서 65를 차감하여 인덱스 번호로 활용
			int column = inputColumn - 1;	// 1 ~ 10 : 0 ~ 9 인덱스 번호로 변환
